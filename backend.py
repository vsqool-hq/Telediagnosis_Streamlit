...
# Słownik priorytetów dla modalności MR i TK
PRIORITY_DICT = {
    'MR': {
        'MR Angiografia': 4,
...
class MedicalVerificationAgent:
    def __init__(self, reference_dataframe, input_folder, output_folder):
        """Inicjalizacja agenta weryfikacji."""
    ...
    def validate_reference_data(self, df):
        ...
        # Dodanie kolumny "Rodzaj procedury sprawdzony" jeśli nie istnieje
    ...
    def extract_number_from_procedure(self, procedure_text):
        if pd.isna(procedure_text): return 1
        match = re.search(r'^(\d+)', str(procedure_text))
        return int(match.group(1)) if match else 1

    def find_recommended_regions(self, rodzaj_procedury, procedura):
        if self.reference_data is None or pd.isna(procedura) or pd.isna(rodzaj_procedury): return None
    ...
    def get_procedure_type_priority(self, modalnosc, rodzaj_procedury):
        """Zwraca priorytet dla danej modalności i rodzaju procedury."""
    ...
    def find_recommended_procedure_type(self, procedura, modalnosc):
        """Znajduje rekomendowany rodzaj procedury z pliku wzorcowego o najniższym priorytecie."""
        ...
        # Znajdź wartość o najwyższym priorytecie (największa liczba)
        best_priority = float('-inf')  # Zaczynamy od minus nieskończoności
        best_rodzaj = None
    ...
    def correct_procedure_types(self, df, col_map):
        """Koryguje rodzaje procedur według priorytetów."""
        corrections_made = []
        ...
        for index in df.index:
            ...
            # Znajdź rekomendowany rodzaj procedury
            ...
            # Zmień tylko jeśli priorytet w pliku wzorcowym jest wyższy (większa liczba = wyższy priorytet)
    ...
    def format_corrected_procedure(self, count):
        if count == 1: return "1 okolica anatomiczna"
    ...
    def process_client_data(self, client_df, client_name):
        ...
        try:
            ...
            # NOWA FUNKCJONALNOŚĆ: Korekta rodzajów procedur według priorytetów
            ...
            # Istniejąca funkcjonalność: Korekta ilości okolic
            ...
            # Zaznaczenie poprawionych wierszy
            ...
            if corrections_made or type_corrections:
                if corrections_made:
                    ...
                    for msg in corrections_made: logs.append(f"  - {msg}")
    ...
    def run_verification(self):
        ...
        try:
            ...
            print(f"Rozpoczynam weryfikację dla {len(tasks)} klientów...", flush=True)

            all_unmatched_records = []
            processed_clients_count = 0

            results = [MedicalVerificationAgent.wrapper_process_client_data(task) for task in tasks]
            ...
            if all_unmatched_records:
                ...
                print(f"Zapisano je do zbiorczego pliku: Rekordy_Bez_Wzorca.xlsx", flush=True)
                print("="*25, flush=True)
...
def bill_extract_multiplier(procedure_text):
    if pd.isna(procedure_text) or procedure_text == "": return 1
    numbers = re.findall(r'\d+', str(procedure_text))
    return int(numbers[0]) if numbers else 1

def bill_map_priority(priority):
    return {'CITO-CITO': 'CITO', 'Pilny': 'PILNE', 'Planowy': 'PLANOWE'}.get(priority, priority)


# ==================================================================================
# NOWA LOGIKA BUDOWANIA KLUCZA CENNIKOWEGO
# Zastępuje dotychczasowe BADANIE_KLUCZ_1 i BADANIE_KLUCZ_2
# ==================================================================================

# Słownik mapowania priorytetu na sufiks cennikowy
PRIORITY_MAP = {
    'CITO-CITO': 'CITO',
    'Pilny': 'PILNE',
    'Planowy': 'PLANOWE',
...
# Słowniki sufixów dla TK wg Rodzaju procedury
TK_SUFFIX_MAP = {
    'TK Zwykłe': '',
...
# Słowa kluczowe do klasyfikacji anatomicznej MR
MR_GLKRG_KEYWORDS = ['głow', 'gło', 'gl ', 'mózg', 'kręgosłup', 'kregosłup', 'kregoslup', 'szyjn', 'piersiow', 'lędźwiow', 'ledzwiow', 'krzyżow', 'krzyzow', 'krg', 'gł']

MR_STAWY_KEYWORDS = ['staw', 'bark', 'kolan', 'biodr', 'łokc', 'lokc', 'nadgarst', 'skok', 'palc', 'stop', 'dłoń', 'dlon', 'ręk', 'rek', 'ramie', 'ramię', 'kończyn', 'konczyn', 'podudz', 'uda', 'udo', 'mięśni', 'miesni', 'szkielet', 'splot', 'pięt', 'piet', 'przedram']


def mr_get_anatomical_category(procedura: str) -> str:
    """Zwraca kategorię anatomiczną MR na podstawie słów kluczowych w nazwie procedury."""
...
def mr_get_type_suffix(rodzaj_procedury: str) -> str:
    """Zwraca sufiks typu (angio/onko/pusty) dla MR."""
...
def build_price_key(row) -> str:
    """
    Buduje klucz do lookupa w cenniku CSV.

    Format kluczy (zgodny z cennikiem):
      TK:  "TK [ANGIO|ONKO] [PORÓWNAWCZE] PRIORYTET [ANGIO|ONKO]"
      MR:  "MR PRIORYTET kat [angio] [PORÓWNAWCZE] [ONKO]"
      RTG: "RTG PRIORYTET"
      MMG: "MMG SKRINING"
    """
    ...
    # --- MMG ---
    if modalnosc == 'MMG':
        # W cenniku jest tylko "MMG SKRINING" – mapujemy oba rodzaje na ten klucz
        return "MMG SKRINING"
    ...
    if modalnosc == 'TK':
        ...
        if porownawcze:
            # Format: "TK PORÓWNAWCZE PRIORYTET [ANGIO|ONKO]"
            # Przykłady z cennika: "TK PORÓWNAWCZE CITO ANGIO", "TK PORÓWNAWCZE PILNE ONKO"
            if type_suffix:
                return f"TK PORÓWNAWCZE {priorytet} {type_suffix}"
            else:
                return f"TK PORÓWNAWCZE {priorytet}"
        else:
            # Format: "TK [ANGIO] PRIORYTET" lub "TK PRIORYTET ONKO"
            # Uwaga: ANGIO jest PRZED priorytetem, ONKO jest PO priorytecie (wg cennika)
            if type_suffix == 'ANGIO':
                return f"TK ANGIO {priorytet}"
            elif type_suffix == 'ONKO':
    ...
    # --- MR ---
    if modalnosc == 'MR':
        kat = mr_get_anatomical_category(procedura)  # GŁ/KRG / stawy / inne
        type_suffix = mr_get_type_suffix(rodzaj)  # angio / ONKO / ''

        # Budujemy segmenty według wzorca z cennika:
        # "MR PRIORYTET kat [angio] [PORÓWNAWCZE] [ONKO]"
    ...
    # Fallback – zwróć sam klucz modalności + priorytet
...
def bill_format_excel_sheet(workbook, sheet_name, data_sections, total_row=None, grand_totals=None):
    ...
    priority_colors = {
        'CITO NA RATUNEK': 'FF6347', 'CITO-UDAR': 'FF4500', 'Bardzo pilny': 'FFA500',
        'CITO-CITO': 'FFC7CE', 'Pilny': 'FFEB9C', 'Planowy': 'C6EFCE'
    }

    # Kolory dla rodzajów procedur (stonowane)
    procedure_type_colors = {
        'MR Onkologia': 'E8D4F0',  # Jasnofioletowy (wspólny dla onkologii)
        'TK Onkologia': 'E8D4F0',  # Jasnofioletowy (wspólny dla onkologii)
        'MR Angiografia': 'D4E8F0',  # Jasnoniebieski (wspólny dla angiografii)
        'TK Angiografia': 'D4E8F0',  # Jasnoniebieski (wspólny dla angiografii)
        'MR Neurologia': 'FFE5CC',  # Jasnopomarańczowy
        'MR Ortopedia': 'D4F0E8',  # Jasna mięta
        'MR Inne - wtym prostata, miednica, jama brzuszna tkanki miekkie': 'F0E8D4',  # Beżowy
        'TK Zwykłe': 'E8E8E8'  # Jasnoszary
    }
    ...
    for section in data_sections:
        ...
        # Kolorowanie nagłówków według priorytetów
        for c_idx, col_name in enumerate(billing_table.columns, 1):
            cell = ws.cell(row=start_row, column=c_idx)
            for p, fill in fills.items():
                if str(col_name).startswith(p): cell.fill = fill; break

        # Kolorowanie wierszy danych według rodzaju procedury
        ...
        for row_idx, (_, row_data) in enumerate(billing_table.iterrows(), start=data_start_row):
            ...
            if rodzaj_procedury in procedure_type_colors:
                row_fill = PatternFill(start_color=procedure_type_colors[rodzaj_procedury],
                                       ...
                                       fill_type='solid')
                for col in range(1, ws.max_column + 1):
                    # Koloruj tylko jeśli komórka nie ma już koloru (priorytetu w nagłówku)
        ...
        for c_idx, col_name in enumerate(billing_table.columns, 1):
            ...
            if any(k in str(col_name) for k in SUMMABLE_KEYWORDS) and pd.api.types.is_numeric_dtype(billing_table.iloc[:, c_idx-1]):
                # Pominąć sumowanie kolumny "Stawka" i "Mnożnik" w podsumowaniach
                if 'Stawka' not in str(col_name) and 'Mnożnik' not in str(col_name):
                    ...
                    if is_currency: sum_cell.style = "accounting"
    ...
    if total_row:
        for c_idx, col_name in enumerate(data_sections[0]['data'].columns, 1):
            if 'Stawka' in str(col_name) or 'Wartość' in str(col_name):
                ...
                if cell.value is not None: cell.style = "accounting"
...
def bill_process_single_file(excel_path, csv_path, output_path):
    ...
    try:
        ...
        # Mnożenie sumy badań porównawczych przez liczbę okolic (mnożnik)
        grouped['Porownawcze_Flag'] = grouped['Porownawcze_Flag'] * grouped['Mnożnik']
        
        #mapped_priority = grouped['Priorytet opisu'].map(bill_map_priority)
        #grouped['BADANIE_KLUCZ_1'] = grouped['Modalność'] + ' ' + mapped_priority
        #grouped['BADANIE_KLUCZ_2'] = grouped['Rodzaj procedury rozlicz.'] + ' ' + mapped_priority

        # NOWY KOD:
        grouped['CENA_KLUCZ'] = grouped.apply(build_price_key, axis=1)

        #merged = grouped.merge(df_prices[['Jednostka', 'BADANIE', 'Cena']],left_on=['Klient', 'BADANIE_KLUCZ_1'],right_on=['Jednostka', 'BADANIE'],how='left')
        #merged = merged.merge(df_prices[['Jednostka', 'BADANIE', 'Cena']],left_on=['Klient', 'BADANIE_KLUCZ_2'],right_on=['Jednostka', 'BADANIE'],how='left',suffixes=('', '_fallback'))
        #merged['Cena'] = merged['Cena'].fillna(merged['Cena_fallback'])
        ...
        #merged.drop(columns=[col for col in ['BADANIE_x', 'Jednostka_x', 'BADANIE_y', 'Jednostka_y', 'Cena_fallback', 'BADANIE_KLUCZ_1', 'BADANIE_KLUCZ_2'] if col in merged.columns], inplace=True)
        ...
        MASTER_PRIORITY_ORDER = ['CITO NA RATUNEK', 'CITO-UDAR', 'Bardzo pilny', 'CITO-CITO', 'Pilny', 'Planowy']
        ...
        with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
            ...
            for m in modalnosci:
                ...
                for c_idx, c_name in enumerate(m_data.columns, 1): ws.cell(row=current_row, column=c_idx).value = c_name
            ...
            for c_idx, c_name in enumerate(billing_table.columns, 1):
                # Dodano pomijanie sumowania dla Mnożnika w sumie całkowitej
...
def run_billing_process(input_dir, cennik_dir, output_dir):
    try:
        ...
        if not csv_files: raise FileNotFoundError(f"Nie znaleziono pliku CSV w folderze Cennik: {cennik_dir}")
        ...
        print(f"Rozpoczynam tworzenie {len(tasks)} rozliczeń...", flush=True)

        results = [bill_process_single_file(*task) for task in tasks]
...
def main(jednostki_dir, wzorcowe_dir, cennik_dir, wynik_dir, sprawdzone_dir):
    for d in [jednostki_dir, wzorcowe_dir, cennik_dir]:
        if not os.path.isdir(d):
            ...
            return
    os.makedirs(wynik_dir, exist_ok=True); os.makedirs(sprawdzone_dir, exist_ok=True)
...
def run_unmatched_only(jednostki_dir, wzorcowe_dir, sprawdzone_dir):
    ...
    # Przygotowanie klucza wzorcowego – unikalne pary (procedura, rodzaj)
    ...
    # Wczytanie pliku wejściowego – OPTYMALIZACJA 1: tylko potrzebne kolumny
    ...
    # Wczytujemy wszystkie kolumny żeby zapisać pełne rekordy w wyniku,
    # ale silnik Excela i tak musi wczytać cały plik – usecols przyspiesza parsowanie
    try:
    ...
    # OPTYMALIZACJA 2: wektorowy join zamiast pętli po wierszach
    # Tworzymy klucze w danych wejściowych
    ...
    # Left join z kluczami wzorcowymi – wiersze bez dopasowania dostaną _matched = NaN
...
# Ta sekcja jest potrzebna do uruchomienia multiprocessing w spakowanej aplikacji
